...
def program_kapat_ve_ac(v2rayexe, goodbydpiexe, dnsredirblacklist):
    ...
    try:
        ...
        subprocess.Popen(f'"{v2rayexe}"', shell=True)  # Çift tırnakla yolu ver
...
def config_dosyasini_yukle(config_dosyasi):
    if os.path.exists(config_dosyasi):
        with open(config_dosyasi, 'r', encoding='utf-8') as config:
            ...
            if len(satirlar) >= 4:
                # 1. 2. 3. ve 4.satırdaki değerleri döndür
                return satirlar[0].strip(), satirlar[1].strip(), satirlar[2].strip(), satirlar[3].strip()
    return None, None, None, None

    
    
def config_dosyasini_kaydet(config_dosyasi, blacklisttxt, v2rayexe, goodbydpiexe, dnsredirblacklist):
    with open(config_dosyasi, 'w+', encoding='utf-8') as config:
        satirlar = config.readlines()
        # Eğer dosya 4 satırdan azsa, dosyaya yeni satırlar ekler
        while len(satirlar) < 4:
            satirlar.append("\n")
            
        # 2. 3. ve 4.satırları güncelle
        satirlar[0] = f"{blacklisttxt}\n"
        satirlar[1] = f"{v2rayexe}\n"
        satirlar[2] = f"{goodbydpiexe}\n"
        satirlar[3] = f"{dnsredirblacklist}\n"
...
if __name__ == "__main__":
    blacklisttxt, v2rayexe, goodbydpiexe, dnsredirblacklist = config_dosyasini_yukle(config_dosyasi)
    ...
    if not goodbydpiexe or not dnsredirblacklist or not v2rayexe or not blacklisttxt:
        blacklisttxt, v2rayexe, goodbydpiexe, dnsredirblacklist = ilk_ayar()
        config_dosyasini_kaydet(config_dosyasi, blacklisttxt, v2rayexe, goodbydpiexe, dnsredirblacklist)
    metin_ekle(blacklisttxt)
    program_kapat_ve_ac(v2rayexe, goodbydpiexe, dnsredirblacklist)